...
public class CustomButton
{
    ...
    public CustomButton(Action OnClick, Func<bool> HasButton, Func<bool> CouldUse, Action OnMeetingEnds, Sprite Sprite,
        Vector3 PositionOffset, HudManager hudManager, string actionName, bool HasEffect, float EffectDuration,
    ...
    public CustomButton(Action OnClick, Func<bool> HasButton, Func<bool> CouldUse, Action OnMeetingEnds, Sprite Sprite,
        Vector3 PositionOffset, HudManager hudManager, string actionName, bool mirror = false, string buttonText = "")
        : this(OnClick, HasButton, CouldUse, OnMeetingEnds, Sprite, PositionOffset, hudManager, actionName, false, 0f,
            () => { }, mirror, buttonText)
    {
    }
    ...
    public void onClickEvent()
    {
        if (this.Timer < 0f && HasButton() && CouldUse())
        {
            ...
            if (Deputy.Instance.HandcuffedKnows.ContainsKey(CachedPlayer.LocalPlayer.PlayerId) &&
                Deputy.Instance.HandcuffedKnows[CachedPlayer.LocalPlayer.PlayerId] > 0f) return;
    ...
    public void Update()
    {
        ...
        {
            OnClick = () => { Deputy.Instance.SetHandcuffedKnows(); };
        }
        else // Reset.
        {
