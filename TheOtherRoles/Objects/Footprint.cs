...
public class FootprintHolder : MonoBehaviour
{
    ...
    private static bool AnonymousFootprints => Singleton<Detective>.Instance.AnonymousFootprint;
    private static float FootprintDuration => Singleton<Detective>.Instance.FootprintDuration;
    ...
    private void Update()
    {
        ...
        foreach (var activeFootprint in _activeFootprints)
        {
            ...
            Color color;
            if (AnonymousFootprints || Singleton<Camouflager>.Instance.CamouflagerTimer > 0)
            {
                color = Palette.PlayerColors[6];
            }
            else if (activeFootprint.Owner == Singleton<Morphling>.Instance.Player && Singleton<Morphling>.Instance.MorphTimer > 0 && Singleton<Morphling>.Instance.MorphTarget != null && Singleton<Morphling>.Instance.MorphTarget.Data != null)
            {
                color = Palette.PlayerColors[Singleton<Morphling>.Instance.MorphTarget.Data.DefaultOutfit.ColorId];
            }
            else
            {
