            ...
            private static bool Prefix(GameData __instance) {
                ...
                foreach (var playerInfo in GameData.Instance.AllPlayers.GetFastEnumerator())
                {
                    if (playerInfo.Object
                        ...
                        || playerInfo.PlayerId == Singleton<Lawyer>.Instance.Player?.PlayerId // Tasks of the Lawyer do not count
                        || (playerInfo.PlayerId == Singleton<Pursuer>.Instance.Player?.PlayerId && Singleton<Pursuer>.Instance.Player.Data.IsDead) // Tasks of the Pursuer only count, if he's alive
                        || playerInfo.PlayerId == Singleton<Thief>.Instance.Player?.PlayerId // Thief's tasks only count after joining crew team as sheriff (and then the thief is not the thief anymore)
                       )
                        continue;
