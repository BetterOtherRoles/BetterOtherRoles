...
{
    private static bool RoleDescriptionIsOpen => roleDescriptionDisplay != null && roleDescriptionDisplay.gameObject != null;
    ...
    private static void Initialize(HudManager hudManager)
    {
        ...
        TheOtherRolesPlugin.Logger.LogMessage($"Number of roles: { allRoleInfo.Count }");
        TheOtherRolesPlugin.Logger.LogMessage($"IsGuesser: { isGuesser }");
        TheOtherRolesPlugin.Logger.LogMessage($"Array Length: { roleDescriptionTMPs.Length }");

        if (vitalsObj == null || Camera.main == null) return;
        
        roleDescriptionDisplay = UnityEngine.Object.Instantiate(vitalsObj.MinigamePrefab, Camera.main.transform, false).Cast<VitalsMinigame>();
