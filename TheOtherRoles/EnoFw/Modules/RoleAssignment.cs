...
public static class RoleAssignment
{
    ...
    private class AssignmentPools
    {
        ...
        public AssignmentPools(AssignmentData data)
        {
            ...
            while (_data.CrewmateRolesCount > 0 && _crewmateRoles.Count > 0 && _data.Crewmates.Count > 0)
            {
                ...
                if (_data.CrewmateRolesCount >= _data.Crewmates.Count || TheOtherRoles.Rnd.Next(0, 100) <= role.SpawnRate)
                {
            ...
            while (_data.NeutralRolesCount > 0 && _neutralRoles.Count > 0 && _data.Neutrals.Count > 0)
            {
                ...
                if (_data.NeutralRolesCount >= _data.Neutrals.Count || TheOtherRoles.Rnd.Next(0, 100) <= role.SpawnRate)
                {
            ...
            while (_data.ImpostorRolesCount > 0 && _impostorRoles.Count > 0 && _data.Impostors.Count > 0)
            {
                ...
                if (_data.ImpostorRolesCount >= _data.Impostors.Count || TheOtherRoles.Rnd.Next(0, 100) <= role.SpawnRate)
                {
    ...
    private class AssignmentData
    {
        ...
        public AssignmentData()
        {
            ...
            var crewmateRolesCount = TheOtherRoles.Rnd.Next(minCrewmateRoles, maxCrewmateRoles);
            var neutralRolesCount = TheOtherRoles.Rnd.Next(minNeutralRoles, maxNeutralRoles);
            var impostorRolesCount = TheOtherRoles.Rnd.Next(minImpostorRoles, maxImpostorRoles);
