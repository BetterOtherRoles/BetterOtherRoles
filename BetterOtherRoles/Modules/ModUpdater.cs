...
namespace BetterOtherRoles.Modules 
{
    public class ModUpdateBehaviour : MonoBehaviour
    {
        ...
        private async Task<bool> DownloadUpdate()
        {
            ...
            var oldFilePath = Path.Combine(Paths.PluginPath, "BetterOtherRoles.dll");
            ...
            if (File.Exists($"{oldFilePath}.old")) File.Delete($"{oldFilePath}.old");
            if (File.Exists(oldFilePath)) File.Move(oldFilePath, $"{oldFilePath}.old");
