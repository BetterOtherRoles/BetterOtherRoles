    ...
    class MeetingHudPatch
    {
        ...
        class MeetingHudBloopAVoteIconPatch
        {
                ...
                [HarmonyArgument(1)] int index, [HarmonyArgument(2)] Transform parent)
            {
                ...
                if (!(!GameOptionsManager.Instance.currentNormalGameOptions.AnonymousVotes ||
                      (CachedPlayer.LocalPlayer.Data.IsDead && TORMapOptions.ghostsSeeVotes) || (DevMode.EnableDevMode && TORMapOptions.ghostsSeeVotes) ||
        ...
        static void guesserOnClick(int buttonTarget, MeetingHud __instance)
        {
            ...
            foreach (var roleInfo in RoleInfo.allRoleInfos)
            {
                ...
                BetterOtherRolesPlugin.Instance.Log.LogDebug($"Role: {roleInfo.name} {(byte)roleInfo.roleId}");
                BetterOtherRolesPlugin.Instance.Log.LogDebug(Rpc.Serialize(roleData.neutralSettings));
                BetterOtherRolesPlugin.Instance.Log.LogDebug(Rpc.Serialize(roleData.impSettings));
                BetterOtherRolesPlugin.Instance.Log.LogDebug(Rpc.Serialize(roleData.crewSettings));
        ...
        class MeetingServerStartPatch
        {
            ...
            {
                BetterOtherRolesPlugin.Logger.LogDebug("MeetingServerStartPatch postfix");
