...
internal static class HttpPatches
{
    ...
    private static class SendWebRequestPatch
    {
        ...
        public static void Prefix(UnityWebRequest __instance)
        {
            ...
            if (path == "/api/games")
            {
                BetterOtherRolesPlugin.Logger.LogDebug($"{__instance.method} {path}");
        ...
        public static void Postfix(UnityWebRequest __instance, UnityWebRequestAsyncOperation __result)
        {
            ...
            if (path == "/api/games")
            {
                __result.add_completed((Action<AsyncOperation>) (_ =>
                {
                    ...
                    {
                        BetterOtherRolesPlugin.Logger.LogDebug("Connected to a modded HTTP matchmaking server");
                    }
                    else
                    {
                        BetterOtherRolesPlugin.Logger.LogWarning("Connected to a vanilla HTTP matchmaking server");
                    }
    ...
    private static class GameStartManagerPatch
    {
        public static void Postfix(GameStartManager __instance)
        {
            ...
            {
                BetterOtherRolesPlugin.Logger.LogWarning("Vanilla region, locking public toggle");
