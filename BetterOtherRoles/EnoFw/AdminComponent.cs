...
public class AdminComponent : MonoBehaviour
{
    ...
    {
        if (SceneManager.GetActiveScene().name != _lastSceneName || !_darkTexture)
        {
    ...
    {
        if (CachedPlayer.LocalPlayer == null || !CustomLobby.IsAdmin(CachedPlayer.LocalPlayer)) return;
    ...
    private static void RenderOptionEditor(CustomOption option, int deep = 0)
    {
        ...
        {
            var value = GUILayout.SelectionGrid(option.SelectionIndex, option.StringSelections.ToArray(), 4);
