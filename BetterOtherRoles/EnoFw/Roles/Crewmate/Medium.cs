...
public class Medium : AbstractRole
{
    ...
    // Options
    public readonly CustomOption QuestionCooldown;
    public readonly CustomOption QuestionDuration;
    public readonly CustomOption OneQuestionPerSoul;
    public readonly CustomOption AdditionalInfoChance;

    public static Sprite SoulSprite => GetSprite("BetterOtherRoles.Resources.Soul.png", 500f);
    public static Sprite QuestionButtonSprite => GetSprite("BetterOtherRoles.Resources.MediumButton.png", 115f);
    ...
    public static string getInfo(PlayerControl target, PlayerControl killer)
    {
        ...
        if (infos.Count > 0)
        {
            var selectedInfo = infos[RolesManager.Rnd.Next(infos.Count)];
            switch (selectedInfo)
            {
        ...
        {
            int randomNumber = RolesManager.Rnd.Next(4);
        ...
        if (RolesManager.Rnd.Next(0, 100) <= Medium.Instance.AdditionalInfoChance)
        {
            int count = 0;
            string condition = "";
            var alivePlayersList = PlayerControl.AllPlayerControls.ToArray().Where(pc => !pc.Data.IsDead);
            switch (RolesManager.Rnd.Next(3))
            {
                case 0:
